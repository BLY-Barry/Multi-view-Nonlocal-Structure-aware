...
def process_epoch(
    ...
    for batch_idx, batch in progress_bar:
        ...
        img2 = img2[valid_im]
        if len(aflow) != 0:
            output = model(img1, img2)  # 前向传播（输入变换后的图像对）
            feat1 = output['feat'][0]  # 描述子特征图
            feat2 = output['feat'][1]
            score1 = output['score'][0]  # 检测分数图
            ...
            # t.save('t.png')
            loss = loss_function(feat1, score1, feat2, score2, aflow, img1, img2)  # 多任务损失计算（描述子、分数、光流场、输入图像）
            optimizer.zero_grad()
            (loss).backward()  # 反向传播
...
def train(args):
    ...
    if args.image_type == 'VIS_SAR':
        training_dataset = MMDataset(args.datapath, args.image_type,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--log_file', type=str, default='Trained/log.txt',
        help='loss logging file'
    )

    parser.add_argument(
        '--name', type=str, default='Trained/',
    ...
    parser.add_argument(
        '--datapath', type=str, default='Evaluation_dataset',
