...
def Random_proj(img1,img2,crop_size=192,ratio=0.2,scale=0.2,rotation=15):
    ...
    #Corners mapping (affine)
        ...
        #calulate the homograph
        M_suf = KGT.get_rotation_matrix2d(torch.FloatTensor([[w/2,h/2]]),angle,scale)  # 旋转缩放矩阵
        M_suf = torch.cat([M_suf,torch.FloatTensor([[[0,0,1]]])],dim=1)
        M = KGT.get_perspective_transform(points_src, points_dst)@M_suf #Homography  单应性矩阵=透视变换矩阵@旋转缩放矩阵
        M = M.to(img1.device)
        ###########################################
