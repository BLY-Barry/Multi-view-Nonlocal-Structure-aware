...
parser.add_argument("--feature_name", type=str, default='ReDFeat_SFOC66+Hard+Hybrid', help='Name of feature')
...
for subset in subsets:
    ...
    for num in [1024]:
        ...
        for id in progress_bar:
            ...
            try:
                suffix = '.12'
                H = scio.loadmat(os.path.join(subset_path, 'test', 'transforms') + '/' + image_list[id].replace('.png',
                                                                                                                suffix + '.mat'))[
                    'H']
            ...
            err = 10
            if len(good) > 4:
                M, mask = cv2.findHomography(src_pts, dst_pts, cv2.RANSAC, ransacReprojThreshold=2.0, maxIters=100000)
